...
async def org_stuctura(callback: CallbackQuery):
   text = ('Чтобы тебе было легче ориентироваться и понимать, как все устроено,' 
           'мы подготовили этот шаг, посвященный нашей <b>организационной структуре.</b>')
   type = 'Орг. структура'
...
async def process_Instructions(callback: CallbackQuery, state: FSMContext):
   try:
      ...
      text_mentor = (
          f'Если ты руководитель отдела, то тебе также может пригодиться инструкция по подбору персонала.'
          f'\nСохрани её и когда у твоего отдела появится потребность'
          f'в новом сотруднике обязательно обратись к данной инструкции. {link_mentor}'
      )
